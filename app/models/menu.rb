class Menu < ApplicationRecord
    attr_accessor :menu_update_status, :pre_menu_name

    #バリデーション
    validates :name, length: { in: 2..12 }
    validates :time, presence: true
    validates :explanation, length: { in: 10..250 }
    validates :category, format: { without: /0000/ }
    validate :name_only # 保存時に カスタムバリデーション発動

    # カスタムバリデーションメソッド
    def name_only
        if self.pre_menu_name != self.name && self.menu_update_status.blank? && Menu.where(hairdresser_id: self.hairdresser_id, name: self.name).present?
